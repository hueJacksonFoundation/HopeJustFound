...
public class GoodsResource {
    ...
    @GetMapping("/goods")
    @Timed
    public List<Goods> getAllGoods(@RequestParam(required = false) String filter) {
        if ("donation-is-null".equals(filter)) {
            log.debug("REST request to get all Goodss where donation is null");
            return StreamSupport
                .stream(goodsRepository.findAll().spliterator(), false)
                .filter(goods -> goods.getDonation() == null)
